...
if __name__ == '__main__':
    ...
    parser.add_argument('--adv_algo', type=str, default='qmix', help='algorithm of the adversaries')
    parser.add_argument('--agent_algo', type=str, default='maddpg', help='algorithm of the agent')
    ...
    policy = Policy(dim_info, args.buffer_capacity, args.batch_size, args.actor_lr, args.critic_lr, result_dir)
            ...
            if step >= args.random_steps and step % args.learn_interval == 0:  # learn every few steps
                for agent_id in env.agents:
                    if 'agent' in agent_id:
                        policy.maddpg_learn(args.batch_size, args.gamma, agent_id)
                policy.qmix_learn(args.batch_size, args.gamma)
                policy.update_target(args.tau)
    ...
    policy.save(episode_rewards)  # save model
